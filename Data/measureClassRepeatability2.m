%% This one was for measuring repeatability index when we train in one
%% position but test in multiple positions (1-16)
...
% combs(6).list = [1,30,235,1051,3704,9742,19806,32717,42563,55416,60414,63949,65175,65459,65532,65535];
combs(6).list = [1,30,234,1124,3728,9742,19806,32717,42584,53100,60401,63949,65175,65482,65532,65535];
...
%         userInfo.settings.enableLPF = 0;
    ...
    for iClass = 1:numClasses 
        ...
        for iPos = 1%posCombs(combs(userList(iUser)).list(1)).comb
...
% end
        ...
        for iClass =  1 : numClasses
            ...
            aa = norm(diag(iS));
            aaa = norm(diag(iStst));
            sigmaRI = sigmaRI + 0.5 * sqrt((iMuTst-iMuTr)*inv(iStst)*(iMuTst-iMuTr)'); %(iS+iStst)/2
            dis = sqrt((iMuTst-iMuTr)*(iMuTst-iMuTr)');
            [aaa aa dis]
        end
